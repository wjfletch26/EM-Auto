 ...
 *
 * This script runs the processor once and verifies the contact is paused in Sheets.
 * By default, it also queues one manual forwarded-reply event first.
 *
 * Usage:
 *   npx tsx scripts/test-reply-forward-pause.ts contact@example.com
 *   npx tsx scripts/test-reply-forward-pause.ts contact@example.com --process-only
 */
...
async function main(): Promise<void> {
  ...
  const flags = new Set(process.argv.slice(3).map((arg) => arg.trim().toLowerCase()));
  const processOnly = flags.has('--process-only') || flags.has('--no-enqueue');
  if (!emailArg) {
    throw new Error(
      'Usage: npx tsx scripts/test-reply-forward-pause.ts contact@example.com [--process-only|--no-enqueue]',
    );
  }
  ...
  if (!processOnly) {
    ...
    console.log(`Process-only mode enabled; skipping enqueue for ${emailArg}.`);
  }
